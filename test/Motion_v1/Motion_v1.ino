...
#include <Metro.h>
#include <EEPROM.h>
#include "Nytinasty_Messages.h"
#include <FastLED.h>
#include <arduinoFFT.h>
#include <WiFi.h>
#include <PubSubClient.h>
#include "Nytinasty_MQTT.h"
...
void showRange() {
  // set color, maximal
  light0(0, N_SENSOR - 1) = CRGB::Green;
  light1(0, N_SENSOR - 1) = CRGB::Blue;

  // scale readings
  uint16_t scaled[N_SENSOR] = {0};
  for (byte i = 0; i < N_SENSOR; i++ ) {
    uint16_t s = constrain(readings.dist[i], readings.noise, readings.max);
    scaled[i] = map(s, readings.noise, readings.max, (uint16_t)1, (uint16_t)255);
